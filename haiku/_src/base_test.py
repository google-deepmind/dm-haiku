...
class BaseTest(parameterized.TestCase):
  ...
  def test_replaceable(self):
    ...
    f._replace(g)  # pytype: disable=attribute-error
    self.assertEqual(f(), "hacked")
    f._reset()  # pytype: disable=attribute-error
    self.assertEqual(f(), "original")

  def test_replaceable_original_property(self):
    def g():
      return "hacked"
    ...
    f._replace(g)  # pytype: disable=attribute-error
    self.assertEqual(f(), "hacked")
    self.assertEqual(f._original(), "original")  # pytype: disable=attribute-error

  def test_replaceable_nested(self):
    def g():
      return "hacked"
    ...
    f._replace(g)  # pytype: disable=attribute-error
    self.assertEqual(f(), "hacked")
    self.assertEqual(h(), "hacked")
    f._reset()  # pytype: disable=attribute-error
    self.assertEqual(f(), "original")
    self.assertEqual(h(), "original")

  @test_utils.transform_and_run
  def test_replaceable_get_parameter(self):

    self.assertEqual(hk.get_parameter("w", [], init=jnp.ones), 1)
    hk.get_parameter._replace(lambda *_, **__: "hacked")  # pytype: disable=attribute-error
    self.assertEqual(hk.get_parameter("w", [], init=jnp.ones), "hacked")
    hk.get_parameter._reset()  # pytype: disable=attribute-error
    self.assertEqual(hk.get_parameter("w", [], init=jnp.ones), 1)

  @test_utils.transform_and_run
  def test_replaceable_set_get_state(self):
    ...
    hk.set_state._replace(lambda *_, **__: "hacked")  # pytype: disable=attribute-error
    self.assertEqual(hk.set_state("i", 1), "hacked")
    hk.set_state._reset()  # pytype: disable=attribute-error

    hk.get_state._replace(lambda *_, **__: "hacked")  # pytype: disable=attribute-error
    self.assertEqual(hk.get_state("i"), "hacked")
    hk.get_state._reset()  # pytype: disable=attribute-error
  ...
  @parameterized.parameters(42, 28)
  def test_replaceable_with_rng(self, seed):
    ctx_key = jax.random.PRNGKey(seed * 2 + 1)
    key = jax.random.PRNGKey(seed)
    _, next_key = jax.random.split(key)
    expected_output = jax.random.uniform(next_key, ())
    ...
    with base.new_context(rng=ctx_key):
      without_decorator_out = jax.random.uniform(hk.next_rng_key(), ()).item()

    with base.new_context(rng=ctx_key):
      with hk.with_rng(key):
        with_decorator_out = jax.random.uniform(hk.next_rng_key(), ()).item()

    self.assertNotEqual(without_decorator_out, expected_output)
    self.assertEqual(with_decorator_out, expected_output)

    hk.with_rng._replace(lambda *_, **__: "hacked")  # pytype: disable=attribute-error
    self.assertEqual(hk.with_rng(key), "hacked")
    hk.with_rng._reset()  # pytype: disable=attribute-error

    hk.next_rng_key._replace(lambda *_, **__: "hacked")  # pytype: disable=attribute-error
    self.assertEqual(hk.next_rng_key(), "hacked")
    hk.next_rng_key._reset()  # pytype: disable=attribute-error
  ...
  @test_utils.transform_and_run
  def test_replaceable_next_rng(self):
    a, b = hk.next_rng_keys(2)
    c, d = hk.next_rng_keys(2)
    for l, r in it.permutations((a, b, c, d), 2):
      self.assertIsNot(l, r)

    hk.next_rng_keys._replace(lambda *_, **__: "hacked")  # pytype: disable=attribute-error
    self.assertEqual(hk.next_rng_keys(2), "hacked")
    hk.next_rng_keys._reset()  # pytype: disable=attribute-error
