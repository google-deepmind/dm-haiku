...
class LiftTest(parameterized.TestCase):
  ...
  def test_replace_lift(self):
    ...
    def outer_f():
      lifted = lift.lift(inner_f.init)
      params = lifted(None)
      self.assertDictEqual(params, {"~": {"w": 0}})
      out = inner_f.apply(params, None)
      return out

    outer = transform.transform(outer_f)
    params = outer.init(None)
    self.assertDictEqual(params, {"lifted/~": {"w": 0}})

    w = outer.apply(params, None)
    self.assertEqual(w, 0)
    self.assertDictEqual(params, {"lifted/~": {"w": 0}})

    def hack(*args, **kwargs):
      raise ValueError("hacked")

    lift.lift._replace(hack)  # pytype: disable=attribute-error
    with self.assertRaisesRegex(ValueError, "hacked"):
      outer = transform.transform(outer_f)
      outer.init(None)
    lift.lift._reset()  # pytype: disable=attribute-error
  ...
  def test_replace_lift_with_state(self):
    ...
    def outer_f():
      lifted, updater = lift.lift_with_state(inner_f.init)
      params, state = lifted(None)
      self.assertEmpty(params)
      out, state = inner_f.apply(params, state, None)
      updater.update(state)
      return out, state

    outer = transform.transform_with_state(outer_f)
    params, state = outer.init(None)
    self.assertEmpty(params)
    self.assertEqual(jax.tree_util.tree_map(int, state), {"lifted/~": {"w": 0}})
    ...
    lift.lift_with_state._replace(hack)  # pytype: disable=attribute-error
    with self.assertRaisesRegex(ValueError, "hacked"):
      outer = transform.transform_with_state(outer_f)
      outer.init(None)
    lift.lift_with_state._reset()  # pytype: disable=attribute-error
